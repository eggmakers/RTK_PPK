 ...
 *
 * Source file: D:\UAVCANread\uavcan移植教程及参考示例\dsdl_compiler\uavcan\protocol\param\Value.uavcan
 */
#include "Value.h"
#include "canard.h"
...
uint32_t uavcan_protocol_param_Value_encode_internal(uavcan_protocol_param_Value* source, void* msg_buf, uint32_t offset, uint8_t root_item)
{
    ...
    // Compound
    offset = uavcan_protocol_param_Empty_encode_internal((void*)&source->empty, msg_buf, offset, 0);
    }
    ...
    {
        canardEncodeScalar(msg_buf, offset, 8, (void*)(source->string_value.data + c));// 255
        offset += 8;
    }
...
/**
  ...
  */
int32_t uavcan_protocol_param_Value_decode_internal(const CanardRxTransfer* transfer, uint16_t payload_len, uavcan_protocol_param_Value* dest, uint8_t** dyn_arr_buf, int32_t offset, uint8_t tao)
{
    int32_t ret = 0;
    uint32_t c = 0;
    ...
    {
    // Compound
    offset = uavcan_protocol_param_Empty_decode_internal(transfer, 0, (void*)&dest->empty, dyn_arr_buf, offset, tao);
    if (offset < 0)
    {
        ret = offset;
        goto uavcan_protocol_param_Value_error_exit;
    ...
    {
        // - Array length 8 bits
        ret = canardDecodeScalar(transfer, offset, 8, false, (void*)&dest->string_value.len); // 255
        if (ret != 8)
        {
            goto uavcan_protocol_param_Value_error_exit;
        }
...
int32_t uavcan_protocol_param_Value_decode(const CanardRxTransfer* transfer, uint16_t payload_len, uavcan_protocol_param_Value* dest, uint8_t** dyn_arr_buf)
{
    ...
    while (1)
    {
        ...
        ret = uavcan_protocol_param_Value_decode_internal(transfer, payload_len, dest, dyn_arr_buf, offset, tao);
